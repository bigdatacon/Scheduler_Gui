...
//    std::cout << "Solver finished" << std::endl;
    ...
    RSchedule* pRSchedule = g_pSolver->GetRSchedule();
    std::vector<std::vector<SUnit*>> units = pRSchedule->GetUnits();
    //add check for null size of units
    if (units.empty()) {
        qDebug() << "Ошибка: units пустой!";
        return;
    }

    std::vector<std::vector<SOperation*>> ms_operations(units.size());

    // Переходим от групп к операциям
    for( int i = 0; i < units.size(); i++ )
    {
        for( int j = 0; j < units[i].size(); j++ )
        {
            if ( units[i][j]->IsGroup () )
            {
                vector<SOperation*> curOpers = ((SGroup*)units[i][j])->GetOperations();

                for( int k = 0; k < curOpers.size(); k++ )
                    ms_operations[i].push_back(curOpers[k]);
            }
            else
                ms_operations[i].push_back((SOperation*)units[i][j]);
        }
    }
    ...
    // чита данные из структур
    // RSchedule* pRSchedule = g_pSolver->GetRSchedule();
    // std::vector<std::vector<RSOperation*>> ms_operations = pRSchedule->GetOperations();

    JSchedule* pJSchedule = g_pSolver->GetJSchedule();
    std::vector<std::vector<SOperation*>> js_operations = pJSchedule->GetOperations();
    ...
    for (int i = 0; i < js_operations.size(); ++i) {
        ...
        if (js_operations[i].empty() || js_operations[i][0]->m_vResourcesIndexes.empty()) {
...
//                i + 1,  // Индекс работы (индекс +1)
            ...
            for (const auto& op : js_operations[i]) 
            {
                ...
                newResourceIndexes.reserve(op->m_vResourcesIndexes.size());

                for (const auto& index : op->m_vResourcesIndexes) {
...
//            vUnavData.push_back({static_cast<int>(resourceIndex), unavStart, unavEnd, unavMachineRect});
    ...
    for (auto &machineOp : m_vMsOperations_cont)
    {
        if ( machineOp.bHighlighted )
        {
            flag = true;
            break;
        }
