...
protected:
    ...
    void CalculateSchedule(const CIndividual &individual, MSchedule* pMSchedule, JSchedule* pJSchedule, ScheduleMetrics* pScheduleMetrics) const;
    bool CalculateResourcesForOperation_R1();

    int GetSetupTime(int nOperationIndex, MSchedule* pMSchedule, vector<int> vResourcesIndexes) const;
    bool SaveScheduleToFileTXT(const string &sFilePath, const MSchedule* pMSchedule, const JSchedule* pJSchedule) const;
    
    int test(const CIndividual &individual, MSchedule* pMSchedule, JSchedule* pJSchedule);
};
