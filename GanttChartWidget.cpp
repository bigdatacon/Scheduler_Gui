...
//1. Выделены жирным группы - ghantchart - bgroup
//2. Убраны стрелки от предшественников - то есть они убираются при клики на пустую область - переделать - flag in drawghantchart
//3. Изменен формат строики состояние - формат отображения данных
//4. Поправлена load data in GhantChart  - попробовать взять ее
...
//    }
    ...
    if (m_bHandToolActive) {
        ...
        QPoint proposedOffset = m_offset + offsetDelta;
        QPoint newOffset = clampOffsetToValidRange(proposedOffset);
...
//        updateScrollbars();
        ...
        for (const auto &unav : m_pGanttChart->getUnavData()) {
            if (unav.rect.contains(pos)) {
                ...
                QString start = (baseTime.addSecs(unav.time_start * 60)).toString("HH:mm");
                QString finish = (baseTime.addSecs(unav.time_end * 60)).toString("HH:mm");
                statusText = QString("%1, Недоступность: %2-%3")
                                .arg(QString::fromStdString(g_pSolver->GetResources()[unav.resource_num - 1]->GetName()))
                                .arg(start)
                                .arg(finish);
                found = true;
                ...
                if (mOp.rect.contains(pos)) {
                    ...
                    for (auto& uc : useCases) 
                    {
                        for (const auto& req : reqResources) 
                        {
                            if (uc.first == req.first && uc.second >= g_pSolver->GetOperations()[mOp.iOperation]->GetDischarge()) 
                            {
                                useCase.append(QString("%1 %2").arg(QString::fromStdString(uc.first)).arg(uc.second));
                                break;
                            }
                        }
                    }

                    statusText = QString("%1, %2, %3: %4-%5")
                                    .arg(QString::fromStdString(resource->GetName()))
                                    .arg(useCase)
                                    .arg(QString::fromStdString(g_pSolver->GetOperations()[mOp.iOperation]->GetID()))
                                    .arg(start)
